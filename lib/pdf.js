...
function generateReportPdfBuffer(report, company, signatureDataUrl) {
  return new Promise((resolve, reject) => {
    ...
    // --- Kostenkalkulation (optional, nur wenn Stunden vorhanden) ---
    const kalk = berechneKalkulation(report);
    if (kalk) {
      doc.moveDown(0.5);
      doc.fillColor(COLORS.label).font('Helvetica-Bold').fontSize(9);
      doc.text('KOSTENKALKULATION', 50, doc.y, { width: 495 });
      doc.moveDown(0.3);
      doc.fillColor(COLORS.text).font('Helvetica').fontSize(10);
      kalk.positionen.forEach((p) => {
        const zeile = `${p.name}: ${p.stunden} Std × ${p.satzText} = ${p.summeText}`;
        doc.text(zeile, 58, doc.y, { width: 480 });
      });
      doc.moveDown(0.3);
      doc.fillColor(COLORS.text).font('Helvetica').fontSize(10);
      doc.text(`Summe netto: ${kalk.nettoText}`, 58, doc.y, { width: 480 });
      doc.text(`zzgl. ${kalk.mwstSatz}% MwSt: ${kalk.mwstText}`, 58, doc.y, { width: 480 });
      doc.fillColor(COLORS.blueprint).font('Helvetica-Bold').fontSize(11);
      doc.text(`Gesamt brutto: ${kalk.bruttoText}`, 58, doc.y, { width: 480 });
      doc.moveDown(0.2);
      doc.fillColor(COLORS.label).font('Helvetica').fontSize(7);
      doc.text('Unverbindliche Kalkulation auf Basis der hinterlegten Stundensaetze. Keine Rechnung.', 58, doc.y, { width: 480 });
      doc.moveDown(0.5);
    }
    ...
    if (signatureDataUrl) {
      try {
        ...
        doc.fillColor(COLORS.label).font('Helvetica-Bold').fontSize(9);
        doc.text('UNTERSCHRIFT KUNDE', 50, doc.y, { width: 495 });
        doc.moveDown(0.4);

        // Unterschrift-Bild einbetten (max 250x100)
        doc.image(sigBuffer, 50, doc.y, { fit: [250, 100], align: 'left' });
        doc.y += 110;
        ...
        doc.text(`Unterschrieben am ${todayStr()} – ${report.kunde || ''}`, 50, doc.y, { width: 300 });
        doc.moveDown(0.5);
      } catch (e) {
        console.error('Unterschrift einbetten fehlgeschlagen:', e.message);
      }
    }
