// Whisper-Transkription fuer Audio und Video-Ton
// Sprachen: Deutsch, Polnisch, Russisch, Aserbaidschanisch - automatische Erkennung
// ffmpeg-static wird verwendet (kein System-ffmpeg noetig)
...
// Konvertiert beliebiges Audio (ogg/opus von WhatsApp) zu MP3 via ffmpeg-static
// MP3 wird zuverlaessig von Whisper akzeptiert
function convertToMp3(inputBuffer, inputExt) {
    ...
    try {
          ...
          execSync(`"${ffmpegPath}" -y -i "${inputPath}" -ar 16000 -ac 1 -b:a 64k "${outputPath}"`, {
                  timeout: 30000,
                  stdio: 'pipe'
          });
...
function extractAudioFromVideo(videoBuffer, videoMime) {
    ...
    try {
          ...
          execSync(`"${ffmpegPath}" -y -i "${inputPath}" -vn -ar 16000 -ac 1 -b:a 64k "${outputPath}"`, {
                  timeout: 60000,
                  stdio: 'pipe'
          });
...
function extractFramesFromVideo(videoBuffer, videoMime, maxFrames = 8) {
    ...
    try {
          fs.writeFileSync(inputPath, videoBuffer);
          execSync(
                  `"${ffmpegPath}" -y -i "${inputPath}" -vf "fps=1/3,scale=1280:-1" -frames:v ${maxFrames} "${framePattern}"`,
...
// Gibt { text, language } zurueck
async function transcribeAudio(audioBuffer, mimeType) {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) throw new Error('OPENAI_API_KEY nicht gesetzt');

  // Erst zu MP3 konvertieren - zuverlaessiger als OGG/Opus direkt
  let mp3Buffer;
    try {
